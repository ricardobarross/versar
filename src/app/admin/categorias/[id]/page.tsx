...
export default function EditarCategoriaPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    supabase.from('categorias').select('*').eq('id', params.id).single()
      .then(({ data }) => {
        if (data) { setNome(data.nome); setAtivo(data.ativo) }
      })
  }, [params.id])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('categorias').update({ nome, slug, ativo }).eq('id', params.id)
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-8">Editar Categoria</h1>
